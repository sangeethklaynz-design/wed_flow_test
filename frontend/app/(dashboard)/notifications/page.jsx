...
function normalizeNotification(n) {
  return {
    ...
    guest: n.guest
      ? {
          ...n.guest,
          status: String(n.guest.status || "pending").toLowerCase(),
          requestForChange: Boolean(n.guest.requestForChange),
        }
      : null,
  };
}
...
export default function NotificationsPage() {
  ...
  // Filters
  const [rsvpChangeRequest, setRsvpChangeRequest] = useState("all"); // requested, not_requested, all
  ...
  const handleMarkAllRead = async () => {
    ...
    if (!token) return;
    try {
      await apiRequest("/api/couple/notifications/mark-read", {
        method: "POST",
        token,
      });
  ...
  const handleDeleteNotification = async () => {
    ...
    if (!token) return;
    try {
      await apiRequest(`/api/couple/notifications/${actionNotification.id}`, {
        method: "DELETE",
        token,
      });
      setNotifications((prev) =>
        prev.filter((n) => n.id !== actionNotification.id)
      );
  ...
  const filteredNotifications = useMemo(() => {
    ...
    if (rsvpChangeRequest === "requested") {
      filtered = filtered.filter(
        (n) => n.guest?.requestForChange === true || n.type === "change_request"
      );
    ...
    if (guestStatus !== "All status") {
      filtered = filtered.filter(
        (n) => n.guest?.status === guestStatus.toLowerCase()
      );
    }

    if (notificationType !== "All types") {
      if (notificationType === "RSVP Change Request") {
        filtered = filtered.filter((n) => n.type === "change_request");
      } else if (notificationType === "New RSVP") {
        filtered = filtered.filter((n) => n.type === "rsvp_submitted");
      } else if (notificationType === "RSVP Declined") {
        filtered = filtered.filter((n) => n.type === "rsvp_cancelled");
      } else if (notificationType === "Guest updates") {
        filtered = filtered.filter((n) =>
          ["guest_added", "guest_updated", "guest_deleted", "invite_resent"].includes(
            n.type
          )
        );
      } else if (notificationType === "Schedule updates") {
        filtered = filtered.filter((n) =>
          ["schedule_added", "schedule_updated", "schedule_deleted"].includes(n.type)
        );
      }
    }
    ...
    if (dateRange === "Last 7 days" || dateRange === "Last 30 days") {
      ...
      filtered = filtered.filter((n) => new Date(n.createdAt).getTime() >= cutoff);
    }
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [rsvpChangeRequest, guestStatus, notificationType, dateRange, sortBy]);
  ...
  const getNotificationIcon = (type) => {
    switch (type) {
      ...
      case "guest_deleted":
        return (
          <div className="w-10 h-10 rounded-full bg-[#fdf5f5] text-[#eb5757] flex items-center justify-center">
      ...
      case "invite_resent":
        return (
          <div className="w-10 h-10 rounded-full bg-[#eef6ff] text-[#2f80ed] flex items-center justify-center">
            <Pencil className="w-5 h-5" />
          </div>
        );
      case "schedule_added":
        return (
          <div className="w-10 h-10 rounded-full bg-[#f0f9f4] text-[#27ae60] flex items-center justify-center">
  ...
  const getTypeBadge = (type) => {
    switch (type) {
      ...
      case "rsvp_cancelled":
        return (
          <span className="px-3 py-1 bg-[#fdf5f5] text-[#eb5757] text-xs font-bold rounded-full">
            RSVP Declined
          </span>
        );
      case "guest_added":
        return (
          <span className="px-3 py-1 bg-[#f0f9f4] text-[#27ae60] text-xs font-bold rounded-full">
            Guest Added
          </span>
        );
      case "guest_updated":
        return (
          <span className="px-3 py-1 bg-[#eef6ff] text-[#2f80ed] text-xs font-bold rounded-full">
            Guest Updated
          </span>
        );
      case "guest_deleted":
    ...
    <div className="flex-1 bg-[#fdfcf9] min-h-screen p-6 md:p-8 w-full">
      ...
      <div className="md:hidden mb-6">
        ...
        <p className="text-muted text-[15px]">
          Stay updated with RSVP changes, guest responses, and important
          activity.
        </p>
      </div>
      ...
      <div className="hidden md:flex justify-between items-center mb-8 bg-white p-5 rounded-2xl border border-border card-shadow">
        <div>
          <h1 className="font-serif font-bold text-2xl text-navy mb-1">
            Notifications
          </h1>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          ...
          <div className="flex items-center gap-4">
            <button
              ...
              className="text-sm text-muted hover:text-navy font-medium"
            >
              Clear all
            </button>
          </div>
        ...
        <div className="flex flex-col xl:flex-row gap-4 w-full">
          ...
          <div className="flex flex-col gap-2 xl:w-[320px] shrink-0">
            ...
            <div className="flex items-center gap-3">
              <button
                onClick={() =>
                  setRsvpChangeRequest(
                    rsvpChangeRequest === "requested" ? "all" : "requested"
                  )
                }
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="relative">
                ...
                >
                  ...
                  <option>Guest updates</option>
                  <option>Schedule updates</option>
                </select>
                <ChevronDown className="w-4 h-4 text-muted absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none" />
              </div>
            </div>
        ...
        {filteredNotifications.length > 0 && (
          <div className="px-6 py-5 flex items-center justify-between border-t border-[#eef0f3]">
            <p className="text-[13px] text-muted font-medium">
              ...
              {Math.min(safePage * itemsPerPage, filteredNotifications.length)}{" "}
